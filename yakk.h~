...
unsigned int YKCtxSwCount = 0; //Type: unsigned int This is an unsigned int that must be incremented each time a context switch occurs, defined as the dispatching of a task other than the task that ran most recently.

unsigned int YKIdleCount = 0; // Type: unsigned int This is an unsigned int that must be incremented by the idle task in its while(1) loop. If desired, the user code can use this value to determine CPU utilization. See the section on YKIdleTask, above, to see how to prevent overflow of YKIdleCount.

unsigned int YKTickNum = 0; //Type: unsigned int This is an unsigned int that must be incremented each time the kernel's tick handler runs.
